...
- [ ] **Base Commands Audit**: Do existing command managers provide the data you need?
  
  ```python
  # Review available base commands in your project:
  from gh_wrapper.commands import (
      CommitsManager,   # Methods: list_commits(), get_commits_for_analysis(), 
                        #          search_commits(), get_commit_details(),
                        #          fetch_commits_from_branch()
      
      RepoManager,      # Methods: list_branches(), get_repo_basics(),
                        #          get_file_structure(), get_recent_commits(),
                        #          get_priority_branches(), get_combined_branches()
      
      PRManager,        # Methods: list_prs(), get_pr_details()
      
      FileManager,      # Methods: get_file_content(), search_in_files()
      
      UsersManager      # Methods: get_user_info()
  )
  ```
  
  **Example Audit Questions:**
  - Need commit history? → Check `CommitsManager.list_commits()` ✅
  - Need file structure? → Check `RepoManager.get_file_structure()` ✅
  - Need PR search? → Check `PRManager.list_prs()` → Filter client-side ⚠️
  - Need workflow data? → Not available → Need new command ❌
  
- [ ] **Review Existing Methods**: Look at what each manager actually provides
  
  ```python
  # Example: What does CommitsManager offer?
  commits_mgr = CommitsManager(executor)
  
  # Available methods you can use:
  commits_mgr.list_commits(branch, limit, path)          # Basic commit list (flattened)
  commits_mgr.get_commits_for_analysis(branch, since)    # Full API response (unflattened)
  commits_mgr.search_commits(query, limit)               # GitHub search API
  commits_mgr.get_commit_details(sha)                    # Single commit details
  commits_mgr.fetch_commits_from_branch(branch, user)    # User-filtered commits
  ```

- [ ] **Determine if New Command Needed**: Only add to Command Layer if:
  - Existing managers don't provide the required data
  - The GitHub CLI tool (`gh`) supports the operation you need
  - Example: Need workflow runs? → `RepoManager` doesn't have it → See [Adding Commands Guide](./adding-commands.md)

- [ ] **Design Pydantic Models**: Plan your feature's output structure
...
# Import existing base command managers
...
class FeatureTracer:
    ...
    Composes existing command managers (FileManager, CommitsManager, PRManager)
    to provide comprehensive cross-sectional analysis by searching:
