...
@pytest.fixture
def mock_pr_manager(mocker: Any) -> Any:
...
def test_analyze_pr_with_diff(mock_pr_manager: Any) -> None:
...
"""
    ...
    pr_manager.get_pr_diff.assert_called_once_with(1, repo="owner/repo")


def test_analyze_pr_with_target_branch(mock_pr_manager: Any) -> None:
...
"""
    ...
    pr_manager.get_pr_diff_against_branch.assert_called_once_with(
        2, "develop", repo="owner/repo"
    )
...
def test_format_as_markdown_empty_output(mock_pr_manager: Any) -> None:
...
def test_format_as_markdown_with_content(mock_pr_manager: Any) -> None:
